import 'dart:ui';

class AppColors {
  static const Color welcomeScreenBackground = Color(0xFF160747);
  static const Color buttonColor = Color(0xFF647AFF);
  static const Color appBackground = Color(0xFFFFFFFF);
  static const Color screenBackground = Color(0xFFFAFAFA);
  static const Color otherLoginOptBackgroundColor = Color(0xFF647AFF);
  static const Color constTextColor = Color(0xFF27252E);
  static const Color iconsCommonColor = Color(0xFF455A64);
  static const Color starIconColor = Color(0xFFFFC817);
  static const Color dtBgColor = Color(0xFFEFF1FA);
  static const Color textColor = Color(0xFF36393C);
}
